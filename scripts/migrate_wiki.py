...
def run_migration(sources: list[str] | None = None, limit: int | None = None) -> None:
    ...
    for src in target_sources:
        ...
        if src_dir.exists():
            md_files = sorted(
                p for p in src_dir.rglob("*.md") if p.is_file() and p.name != "index.md"
            )
    ...
    logger.info(
        "ingestion_phase_complete", total_docs=len(all_docs), total_pages=total_pages_created
    )

    # Linker phase
    link_res = linker.link_all_pages()
    logger.info(
        "linking_phase_complete",
        pages_processed=link_res["pages_processed"],
        links_updated=link_res["total_links"],
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Initial C64 LLM-Wiki non-destructive data migration script"
    )
    parser.add_argument(
        "--sources", nargs="+", help="Sources to migrate (e.g. c64ref codebase_c64_org)"
    )
