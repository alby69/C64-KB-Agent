...
- Decide on a port number (from 1 to 254) that is unique to your particular competition on the local network. You can run multiple multiplayer competitions on the same network, as long as you give each competition a unique port number for it to communicate on. Port numbers 0, 84, 153, 156, 158-160, 175-179, 208-223 and 255 are reserved by Econet and should not be used (see the [RISC OS 3 PRM for details](http://www.riscos.com/support/developers/prm/econet.html#42083)).
...
- Now that the scoreboard is set up, it's time to configure the individual games of Elite that want to take part. We'll start by looking at 8-bit clients (i.e. the BBC Micro, BBC Master and Acorn Electron); see the last point below for information on the Archimedes version.
...
- As part of the automated page-turning feature, you can save the current scoreboard screen to a file after each page turn. This can then be loaded by anyone wanting to see the scores. You can optionally enter a full path for the screen file - just make sure the user running the scoreboard has permission to write to that filename. The scoreboard will set the file's access rights to WR/R, so other users can read the file.
 To load the screen into a BBC Micro, switch to screen mode 7 and simply *LOAD it (note you must turn off any second processors for this to work). Here's a simple BASIC program that reloads the screen when you press a key:`MODE 7 INPUT "Enter screen filename: " file$ REPEAT CLS OSCLI("LOAD "+file$+" 7C00") A=GET UNTIL FALSE`You can also use an Archimedes to view the scoreboard file, but this time you need to switch to screen mode 7 and do a *TYPE. Here's the same program, but for RISC OS (the only difference is in the OSCLI statement):`MODE 7 INPUT "Enter screen filename: " file$ REPEAT CLS OSCLI("TYPE "+file$) A=GET UNTIL FALSE`The screen file is only 1000 bytes, so this should be faster and more reliable than using *VIEW.
...
- Convert a score file to TSV. This takes a score file (as saved by the scoreboard) and converts it into TSV format, which is suitable for loading into a spreadsheet for analysis.
- Merge two score files. This takes two score files (as saved by the scoreboard) and concatenates them. You can then load the result into the scoreboard if you need to delete any duplicates. If the two files together contain more than 100 players, then not all of the second file will be included in the result, as each score file can only support 100 players; in this case you might want to edit the files in the scoreboard before merging them.
