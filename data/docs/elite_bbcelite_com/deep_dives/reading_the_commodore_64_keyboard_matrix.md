---
title: Reading the Commodore 64 keyboard matrix
...
In the case of the Commodore 64 version of Elite, the keyboard works rather differently to the original BBC Micro version. For the latter, the authors implemented a key logging system to get around limitations in the operating system's keyboard routines, so they could support lots of keys being pressed at once; see the deep dive on [the key logger](https://elite.bbcelite.com/the_key_logger.html) for details. The Commodore 64 version still uses this key logger, as its built-in keyboard routines are similarly limited, but it is structured and populated in a very different way to the BBC Micro, because the Commodore 64 keyboard is not the same.
...
The key logger is at [KEYLOOK](https://elite.bbcelite.com/c64/main/workspace/keylook.html) (which shares its address with the label KLO), and interspersed throughout the logger are the labels that the original BBC Micro version uses for its diminutive 16-byte table. On the BBC, the status of the primary flight keys are stored in variables [KY1](https://elite.bbcelite.com/cassette/main/workspace/zp.html#ky1) through [KY7](https://elite.bbcelite.com/cassette/main/workspace/zp.html#ky7), so KEYLOOK has the same labels, they're just dotted out throughout the much larger key logger. For example, [KY1 is at KLO+$9](https://elite.bbcelite.com/c64/main/workspace/keylook.html#ky1), and [KY7 is at KLO+$36](https://elite.bbcelite.com/c64/main/workspace/keylook.html#ky7).
...
For more information on how the Commodore 64's keyboard works, check out this [really deep dive](https://c64os.com/post/howthekeyboardworks).

---
*Fonte originale: [https://elite.bbcelite.com/deep_dives/reading_the_commodore_64_keyboard_matrix.html](https://elite.bbcelite.com/deep_dives/reading_the_commodore_64_keyboard_matrix.html)*
