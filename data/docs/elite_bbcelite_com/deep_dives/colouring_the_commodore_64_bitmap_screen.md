---
title: Colouring the Commodore 64 bitmap screen
...
- For the space view, the bitmap is one bit per pixel, and just like the BBC Micro's mode 4, the bitmap is split up into character blocks of eight rows, with eight pixels on each row in the character block.
- For the dashboard, the bitmap is two bits per pixel, and just like the BBC Micro's mode 5, the bitmap is split up into character blocks of eight rows, with four pixels on each row in the character block.

See the deep dive on [drawing pixels in the Commodore 64 version](https://elite.bbcelite.com/drawing_pixels_in_the_commodore_64_version.html) for more information.
...
When we need to display the space view and the dashboard, the [wantdials](https://elite.bbcelite.com/c64/main/subroutine/wantdials.html) routine changes abraxas to $91, and setting VIC-II register $18 to this value sets the address of screen RAM to offset $2400 within the VIC-II bank at $4000, so the screen RAM address for the lower part of the screen gets changed to $6400.
...
The palettes for the dashboard in screen RAM are set up in [part 5 of the game loader](https://elite.bbcelite.com/c64/game_loader/subroutine/elite_loader_part_5_of_7.html), and colour RAM is set up in [part 6](https://elite.bbcelite.com/c64/game_loader/subroutine/elite_loader_part_6_of_7.html). The code simply copies data from the [sdump](https://elite.bbcelite.com/c64/game_loader/variable/sdump.html) and [cdump](https://elite.bbcelite.com/c64/game_loader/variable/cdump.html) variables into screen RAM and colour RAM, and in the BeebAsm version of the source on this site, these variables are simply blocks of data.
...
---
*Fonte originale: [https://elite.bbcelite.com/deep_dives/colouring_the_commodore_64_bitmap_screen.html](https://elite.bbcelite.com/deep_dives/colouring_the_commodore_64_bitmap_screen.html)*
