...
c64ref:
  ...
  - name: Original Disassembly
    author: —
    description: '- **$AC0F**: set input mode flag, $00 = INPUT, $40 = GET, $98 =
      READ'
  - name: Bob Sander-Cederlof
    author: Bob Sander-Cederlof
    description: '- **$AC11**: ADDRESS OF INPUT STRING'
---
...
.AC2F  D0 20    BNE $AC51   ; branch if not null pointer was to null entry
.AC31  24 11    BIT $11   ; test input mode flag, $00 = INPUT, $40 = GET, $98 = READ
.AC33  50 0C    BVC $AC41   ; branch if not GET else was GET
...
.AC41  30 75    BMI $ACB8   ; branch if READ else was INPUT
...
.AC5A  50 09    BVC $AC65   ; branch if not GET else do string GET
.AC5C  E8       INX   ; clear X ??
.AC5D  86 7A    STX $7A   ; save BASIC execute pointer low byte
.AC5F  A9 00    LDA #$00   ; clear A
.AC61  85 07    STA $07   ; clear search character
.AC63  F0 0C    BEQ $AC71   ; branch always is string INPUT or string READ
.AC65  85 07    STA $07   ; save search character
.AC67  C9 22    CMP #$22   ; compare with "
.AC69  F0 07    BEQ $AC72   ; branch if quote string is not in quotes so ":", "," or $00 are the termination characters
...
.AC78  69 00    ADC #$00   ; add to pointer low byte. this add increments the pointer if the mode is INPUT or READ and the data is a "..." string
...
.AC86  4C 91 AC JMP $AC91   ; continue processing command GET, INPUT or READ is numeric
...
.AC9A  4C 4D AB JMP $AB4D   ; else go do bad input routine string terminated with ":", "," or $00
...
.ACB5  4C 15 AC JMP $AC15   ; go READ or INPUT next variable from list was READ
...
- **$AC13**: save READ pointer high byte READ, GET or INPUT next variable from list
...
- **$AC2C**: scan memory
- **$AC2F**: branch if not null pointer was to null entry
- **$AC31**: test input mode flag, $00 = INPUT, $40 = GET, $98 = READ
- **$AC33**: branch if not GET else was GET
...
- **$AC41**: branch if READ else was INPUT
...
- **$AC5A**: branch if not GET else do string GET
- **$AC5C**: clear X ??
- **$AC5D**: save BASIC execute pointer low byte
- **$AC5F**: clear A
- **$AC61**: clear search character
- **$AC63**: branch always is string INPUT or string READ
- **$AC65**: save search character
- **$AC67**: compare with "
- **$AC69**: branch if quote string is not in quotes so ":", "," or $00 are the termination characters
- **$AC6B**: set ":"
...
- **$AC78**: add to pointer low byte. this add increments the pointer if the mode is INPUT or READ and the data is a "..." string
...
- **$AC86**: continue processing command GET, INPUT or READ is numeric
...
- **$AC96**: compare with ","
- **$AC98**: branch if ","
- **$AC9A**: else go do bad input routine string terminated with ":", "," or $00
...
- **$ACB5**: go READ or INPUT next variable from list was READ
...
- **$AC11**: ADDRESS OF INPUT STRING
- **$AC15**: GET ADDRESS OF VARIABLE
- **$AC1C**: SAVE CURRENT TXTPTR,
- **$AC1E**: WHICH POINTS INTO PROGRAM
- **$AC24**: SET TXTPTR TO POINT AT INPUT BUFFER
- **$AC26**: OR "DATA" LINE
- **$AC2C**: GET CHAR AT PNTR
- **$AC2F**: NOT END OF LINE OR COLON
- **$AC31**: DOING A "GET"?
- **$AC33**: NO
- **$AC35**: YES, GET CHAR
- **$AC3F**: ...ALWAYS
- **$AC41**: DOING A "READ"
- **$AC47**: DOING AN "INPUT", PRINT "?"
- **$AC4A**: PRINT ANOTHER "?", AND INPUT A LINE
- **$AC51**: GET NEXT INPUT CHAR
- **$AC54**: STRING OR NUMERIC?
- **$AC56**: NUMERIC
- **$AC58**: STRING -- NOW WHAT INPUT TYPE?
- **$AC5A**: NOT A "GET"
- **$AC5C**: "GET"
- **$AC61**: NO OTHER TERMINATORS THAN $00
- **$AC63**: ...ALWAYS
- **$AC67**: TERMINATE ON $00 OR QUOTE
- **$AC6B**: TERMINATE ON $00, COLON, OR COMMA
- **$AC78**: SKIP OVER QUOTATION MARK, IF
- **$AC7A**: THERE WAS ONE
- **$AC7D**: BUILD STRING STARTING AT (Y,A) TERMINATED BY $00, (CHARAC), OR (ENDCHR)
- **$AC80**: SET TXTPTR TO POINT AT STRING
- **$AC83**: STORE STRING IN VARIABLE
- **$AC89**: GET FP NUMBER AT TXTPTR
- **$AC8E**: STORE RESULT IN VARIABLE
- **$AC94**: END OF LINE OR COLON
- **$AC96**: COMMA IN INPUT?
- **$AC98**: YES
- **$AC9A**: NOTHING ELSE WILL DO
- **$AC9D**: SAVE POSITION IN INPUT BUFFER
- **$ACA5**: RESTORE PROGRAM POINTER
- **$ACAD**: NEXT CHAR FROM PROGRAM
- **$ACB0**: END OF STATEMENT
- **$ACB2**: BETTER BE A COMMA THEN
- **$ACB8**: GET OFFSET TO NEXT COLON OR EOL
- **$ACBB**: TO FIRST CHAR OF NEXT LINE
- **$ACBC**: WHICH:  EOL OR COLON?
- **$ACBD**: COLON
- **$ACBF**: EOL: MIGHT BE OUT OF DATA
- **$ACC1**: CHECK HI-BYTE OF FORWARD PNTR
- **$ACC2**: END OF PROGRAM?
- **$ACC4**: YES, WE ARE OUT OF DATA
- **$ACC6**: PICK UP THE LINE #
- **$ACCE**: POINT AT FIRST TEXT CHAR IN LINE
- **$ACD1**: GET 1ST TOKEN OF STATEMENT
- **$ACD7**: SAVE TOKEN IN X-REG
- **$ACD8**: DID WE FIND A "DATA" STATEMENT?
- **$ACDA**: NOT YET
- **$ACDC**: YES, READ IT NO MORE INPUT REQUESTED
- **$ACDF**: GET POINTER IN CASE IT WAS "READ"
- **$ACE3**: "READ" OR "INPUT"?
- **$ACE5**: "INPUT"
- **$ACE7**: "DATA", SO STORE (Y,X) AT DATPTR
- **$ACEA**: "INPUT":  ANY MORE CHARS ON LINE?
- **$ACEE**: NO, ALL IS WELL
- **$ACF0**: YES, ERROR
- **$ACF6**: "EXTRA IGNORED"
- **$ACFC**: '?extra ignored'
