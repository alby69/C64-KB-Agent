---
title: Background Color 1
source_url: https://github.com/mist64/c64ref/blob/main/src/c64io/mapping_the_commodore_64.txt
category: reference
topics:
- io-map
- vic-ii-registers
difficulty: intermediate
language: assembly
hardware:
- VIC-II
related: []
scraped_at: '2026-07-29'
c64ref:
  module: c64io
  source_files:
  - mapping_the_commodore_64.txt
  - c64_programmer's_reference_guide.txt
  address: $D022
  symbol: BGCOL1
  sources:
  - name: C64 Programmer's Reference Guide
    author: Commodore
    description: Background Color 1
  - name: Mapping the Commodore 64
    author: Sheldon Leemon
    description: This register sets the color for the 01 bit-pair of multicolor
---

# BGCOL1 — Background Color 1 ($D022)

## Panoramica
Il registro o area di memoria BGCOL1 è descritto in dettaglio di seguito.

## Dettagli Tecnici
- **Indirizzo**: `$D022` (`53282` decimale)
- **Range**: `$D022`
- **Dimensione**: `1 byte`
- **Permessi**: `R/W`

## Descrizioni per Fonte

### C64 Programmer's Reference Guide (Commodore)
Background Color 1

### Mapping the Commodore 64 (Sheldon Leemon)
This register sets the color for the 01 bit-pair of multicolor
     character graphics, and the background color for characters having
     screen codes 64-127 in extended background color text mode.  The
     default color value is 1 (white).

---
*Fonte: [c64ref](https://github.com/mist64/c64ref) — Ultimate Commodore 64 Reference*